...
alpha = param(50); %theta_SIRT = param(51);
...
theta_SIRT=1;
activeSIRT1 = theta_SIRT * SIRT1;
AC_FREE_SIRT1 = k_AC_free_SIRT1*activeSIRT1/(activeSIRT1+Ja_SIRT1_HIF1a)*HIF1a_AC*NAD;
%0428
...
SIRT1_B = k_bind_SIRT1*NAM*activeSIRT1;
SIRT1_UB = k_unbind_SIRT1*SIRT1__NAM;

NAM_F = k_NAM_f*pAMPK/(pAMPK+Ja_NAM_f_pAMPK)*NAM; %0905
NAM_B = k_NAM_b*activeSIRT1/(activeSIRT1+Ja_NAM_b_SIRT1)*NAD;
...
dO2dt = k_input_O2 - 1/2 * ETC_F + 1/2 * ETC_B - O2_ROS - NOX_F + NOX_B;
dpAMPKdt = AMPK_P - AMPK_UP;

dROSdt = O2_ROS + NOX_F - NOX_B - kd_ROS * pAMPK/(pAMPK + Ja_SCAV_pAMPK) * SCAV * ROS;
...
dNADdt = ETC_F - ETC_B + NOX_F - NOX_B + NAM_F - NAM_B + NAD_F - NAD_B + FREE_AC_P300 - AC_FREE_SIRT1;

dNADHdt = -ETC_F + ETC_B - NOX_F + NOX_B -NAD_F + NAD_B;


dSIRT1dt = ks1_SIRT1 +ks2_SIRT1 * HIF1 - kd_SIRT1*SIRT1 + SIRT1_UB - SIRT1_B;
